...
// DongengCategory represents a hierarchical category for dongeng (stories).
// Top-level categories have ParentID == nil.
// Sub-categories have ParentID pointing to their parent.
type DongengCategory struct {
	...
	Children  []DongengCategory `gorm:"foreignKey:ParentID"                           json:"children,omitempty"`
	CreatedAt time.Time         `json:"created_at"`
	UpdatedAt time.Time         `json:"updated_at"`
	IsDeleted bool              `gorm:"column:is_deleted;not null;default:false"       json:"-"`
}

// FindTopLevelDongengCategories returns all non-deleted top-level categories
// (ParentID IS NULL) with their sub-categories preloaded.
func FindTopLevelDongengCategories(db *gorm.DB) ([]DongengCategory, error) {
	var cats []DongengCategory
	err := db.
