...
func TestGetFairyTales_FilterByCategory(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(*) FROM "dongengs" WHERE is_deleted = $1 AND dongeng_category_id = $2`)).
		WithArgs(false, categoryID).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	mock.ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "dongengs" WHERE is_deleted = $1 AND dongeng_category_id = $2 LIMIT $3`)).
		WithArgs(false, categoryID, 10).
